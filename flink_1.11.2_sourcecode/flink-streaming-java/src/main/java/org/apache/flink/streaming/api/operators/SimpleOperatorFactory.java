...
public class SimpleOperatorFactory<OUT> extends AbstractStreamOperatorFactory<OUT> {
	...
	public static <OUT> SimpleOperatorFactory<OUT> of(StreamOperator<OUT> operator) {
		...
		// TODO 注释： StreamSink
		else if(operator instanceof StreamSink && ((StreamSink) operator).getUserFunction() instanceof OutputFormatSinkFunction) {
			return new SimpleOutputFormatOperatorFactory<>((StreamSink) operator);
		}

		// TODO 注释： AbstractUdfStreamOperator
		else if(operator instanceof AbstractUdfStreamOperator) {
			return new SimpleUdfStreamOperatorFactory<OUT>((AbstractUdfStreamOperator) operator);
		}

		// TODO 注释： SimpleOperatorFactory
		else {
	...
	public <T extends StreamOperator<OUT>> T createStreamOperator(StreamOperatorParameters<OUT> parameters) {
		...
		if(operator instanceof SetupableStreamOperator) {
			((SetupableStreamOperator) operator).setup(parameters.getContainingTask(), parameters.getStreamConfig(), parameters.getOutput());
		}
		return (T) operator;
	}
