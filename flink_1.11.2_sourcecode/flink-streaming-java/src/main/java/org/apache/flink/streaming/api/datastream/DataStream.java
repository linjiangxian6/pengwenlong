...
/**
 * // TODO 注释： DataStream表示相同类型的元素流。
 * // TODO 注释： 通过应用转换，可以将一个DataStream转换为另一个DataStream
 * A DataStream represents a stream of elements of the same type.
 * A DataStream can be transformed into another DataStream by applying a transformation as
 * for example:
 * <ul>
...
public class DataStream<T> {
	...
	public <R> SingleOutputStreamOperator<R> process(ProcessFunction<T, R> processFunction) {
		...
		TypeInformation<R> outType = TypeExtractor
			.getUnaryOperatorReturnType(processFunction, ProcessFunction.class, 0, 1, TypeExtractor.NO_INDEX, getType(), Utils.getCallLocationName(), true);
	...
	public SingleOutputStreamOperator<T> assignTimestampsAndWatermarks(WatermarkStrategy<T> watermarkStrategy) {
		...
		final TimestampsAndWatermarksOperator<T> operator = new TimestampsAndWatermarksOperator<>(cleanedStrategy);
