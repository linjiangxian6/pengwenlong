...
/**
 * // TODO 注释： 所有 StreamTask 的基类。
 * // TODO 注释： Task 是由 TaskManager 部署并执行的本地处理单元。
 * // TODO 注释： 每个 Task 运行一个或多个{@link StreamOperator}，这些 StreamOperator 形成 Task 的 OperatorChain
 * // TODO 注释： 链接在一起的 Operators 在同一线程中并因此在同一 stream partition 上同步执行。
 * // TODO 注释： 这些链的常见情况是连续的 map / flatmap / filter 任务。
 ...
 *
 * // TODO 注释： 任务链包含一个 "head" operator 和多个 chained operators
 * // TODO 注释： StreamTask 用 head operator 来定义，分为 one-input 和 two-input两种 Task，以及 sources，iteration heads 和 iteration tails。
 * <p>The task chain contains one "head" operator and multiple chained operators.
 * The StreamTask is specialized for the type of the head operator: one-input and two-input tasks,
 * as well as for sources, iteration heads and iteration tails.
 *
 * // TODO 注释： Task类负责处理由头运算符读取的流以及在运算符链末端由运算符产生的流的设置。请注意，链条可能会分叉，因此具有多个末端。
...
public abstract class StreamTask<OUT, OP extends StreamOperator<OUT>> extends AbstractInvokable implements AsyncExceptionHandler {
	...
	protected StreamTask(Environment environment, @Nullable TimerService timerService, Thread.UncaughtExceptionHandler uncaughtExceptionHandler,
		...
		this.subtaskCheckpointCoordinator = new SubtaskCheckpointCoordinatorImpl(stateBackend.createCheckpointStorage(getEnvironment().getJobID()), getName(),
			actionExecutor, getCancelables(), getAsyncOperationsThreadPool(), getEnvironment(), this, configuration.isUnalignedCheckpointsEnabled(),
	...
	private void readRecoveredChannelState() throws IOException, InterruptedException {
		...
		if(inputGates != null && inputGates.length > 0) {
			...
			CompletableFuture.allOf(futures).thenRun(() -> mainMailboxExecutor.execute(this::requestPartitions, "Input gates request partitions"));
		}
	}
	...
	private boolean triggerCheckpoint(CheckpointMetaData checkpointMetaData, CheckpointOptions checkpointOptions,
		...
		try {
			...
			// TODO 注释： 如果未成功，则取消本次 checkpoint
			if(!success) {
	...
	private boolean performCheckpoint(CheckpointMetaData checkpointMetaData, CheckpointOptions checkpointOptions, CheckpointMetrics checkpointMetrics,
	...
	protected void declineCheckpoint(long checkpointId) {
		getEnvironment()
			.declineCheckpoint(checkpointId, new CheckpointException("Task Name" + getName(), CheckpointFailureReason.CHECKPOINT_DECLINED_TASK_NOT_READY));
	}
	...
	public Future<Void> notifyCheckpointAbortAsync(long checkpointId) {
		return notifyCheckpointOperation(() -> subtaskCheckpointCoordinator.notifyCheckpointAborted(checkpointId, operatorChain, this::isRunning),
	...
	private static <OUT> RecordWriter<SerializationDelegate<StreamRecord<OUT>>> createRecordWriter(StreamEdge edge, int outputIndex, Environment environment,
		String taskName, long bufferTimeout) {
		...
		RecordWriter<SerializationDelegate<StreamRecord<OUT>>> output = new RecordWriterBuilder<SerializationDelegate<StreamRecord<OUT>>>()
			.setChannelSelector(outputPartitioner).setTimeout(bufferTimeout).setTaskName(taskName).build(bufferWriter);
