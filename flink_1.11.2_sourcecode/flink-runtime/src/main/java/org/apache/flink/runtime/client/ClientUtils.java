...
public enum ClientUtils {
	...
	public static void extractAndUploadJobGraphFiles(JobGraph jobGraph,
		...
		Collection<Tuple2<String, Path>> userArtifacts = jobGraph.getUserArtifacts().entrySet().stream()
			.map(entry -> Tuple2.of(entry.getKey(), new Path(entry.getValue().filePath))).collect(Collectors.toList());
	 ...
	 * @throws FlinkException if the upload fails
	 */
	public static void uploadJobGraphFiles(JobGraph jobGraph, Collection<Path> userJars,
	 ...
	 * @throws IOException if the upload fails
	 */
	private static void uploadAndSetUserArtifacts(JobGraph jobGraph, Collection<Tuple2<String, Path>> artifactPaths,
	...
	private static Collection<Tuple2<String, PermanentBlobKey>> uploadUserArtifacts(JobID jobID, Collection<Tuple2<String, Path>> userArtifacts,
		BlobClient blobClient) throws IOException {
