...
public class BatchTask<S extends Function, OT> extends AbstractInvokable implements TaskContext<S, OT> {
				...
				// set the iterative status for inputs and broadcast inputs
				{
					...
					for(int i = 0; i < numInputs; i++) {
						...
						if(numberOfEventsUntilInterrupt < 0) {
							...
							if(LOG.isDebugEnabled()) {
								LOG.debug(formatLogString(
									"Input [" + i + "] reads in supersteps with [" + +numberOfEventsUntilInterrupt + "] event(s) till next superstep."));
							}
						}
					}
...

				{
					...
					for(int i = 0; i < numBroadcastInputs; i++) {
						...
						if(numberOfEventsUntilInterrupt < 0) {
							...
							if(LOG.isDebugEnabled()) {
								LOG.debug(formatLogString(
									"Broadcast input [" + i + "] reads in supersteps with [" + +numberOfEventsUntilInterrupt + "] event(s) till next superstep."));
							}
						}
					}
	...
	protected void initialize() throws Exception {
		...
		try {
			...
			throw new Exception(
				"The driver setup for '" + this.getEnvironment().getTaskInfo().getTaskName() + "' , caused an error: " + t.getMessage(), t);
		}
	...
	protected <X> void readAndSetBroadcastInput(int inputNum, String bcVarName, DistributedRuntimeUDFContext context,
		int superstep) throws IOException {
		...
		BroadcastVariableMaterialization<X, ?> variable = getEnvironment().getBroadcastVariableManager()
			.materializeBroadcastVariable(bcVarName, superstep, this, reader, serializerFactory);
	...
	protected void run() throws Exception {
		...
		try {
			...
			try {
				...
				throw new Exception(
					"The data preparation for task '" + this.getEnvironment().getTaskInfo().getTaskName() + "' , caused an error: " + t.getMessage(),
					t);
			}
		...
		try {
			...
			if(stubSuperClass != null && !stubSuperClass.isAssignableFrom(stub.getClass())) {
				throw new RuntimeException(
					"The class '" + stub.getClass().getName() + "' is not a subclass of '" + stubSuperClass.getName() + "' as is required.");
			}
	...
	protected void initLocalStrategies(int numInputs) throws Exception {
		...
		for(int i = 0; i < numInputs; i++) {
			...
			if(async) {
				@SuppressWarnings({"unchecked", "rawtypes"}) TempBarrier<?> barrier = new TempBarrier(this, getInput(i), this.inputSerializers[i],
					memMan, ioMan, memoryPages);
				barrier.startReading();
				this.tempBarriers[i] = barrier;
				this.inputs[i] = null;
			} else if(cached) {
				@SuppressWarnings({"unchecked", "rawtypes"}) SpillingResettableMutableObjectIterator<?> iter = new SpillingResettableMutableObjectIterator(
			...
			if(this.excludeFromReset[i]) {
				...
				if(this.inputIsCached[i]) {
					...
					if(this.inputIsAsyncMaterialized[i]) {
						...
						@SuppressWarnings({"unchecked", "rawtypes"}) TempBarrier<?> barrier = new TempBarrier(this, getInput(i),
							this.inputSerializers[i], memMan, ioMan, pages);
						barrier.startReading();
						this.tempBarriers[i] = barrier;
						this.inputs[i] = null;
					}
		...
		if(localStrategy != null) {
			switch(localStrategy) {
				case NONE:
					...
					break;
				case SORT:
					@SuppressWarnings({"rawtypes", "unchecked"}) UnilateralSortMerger<?> sorter = new UnilateralSortMerger(getMemoryManager(),
						getIOManager(), this.inputIterators[inputNum], this, this.inputSerializers[inputNum], getLocalStrategyComparator(inputNum),
				...
				case COMBININGSORT:
					...
					try {
						...
						throw new RuntimeException(
							"Initializing the user code and the configuration failed" + (e.getMessage() == null ? "." : ": " + e.getMessage()), e);
					}
					...
					@SuppressWarnings({"rawtypes", "unchecked"}) CombiningUnilateralSortMerger<?> cSorter = new CombiningUnilateralSortMerger(
						(GroupCombineFunction) localStub, getMemoryManager(), getIOManager(), this.inputIterators[inputNum], this,
						this.inputSerializers[inputNum], getLocalStrategyComparator(inputNum), this.config.getRelativeMemoryInput(inputNum),
						this.config.getFilehandlesInput(inputNum), this.config.getSpillingThresholdInput(inputNum),
						this.getTaskConfig().getUseLargeRecordHandler(), this.getExecutionConfig().isObjectReuseEnabled());
	...
	protected MutableObjectIterator<?> createInputIterator(MutableReader<?> inputReader, TypeSerializerFactory<?> serializerFactory) {
		...
		@SuppressWarnings({"unchecked", "rawtypes"}) final MutableObjectIterator<?> iter = new ReaderIterator(reader,
	...
	public DistributedRuntimeUDFContext createRuntimeContext(MetricGroup metrics) {
		...
		return new DistributedRuntimeUDFContext(env.getTaskInfo(), getUserCodeClassLoader(), getExecutionConfig(), env.getDistributedCacheEntries(),
		...
		if(this.inputs[index] != null) {
			...
			try {
				if(this.tempBarriers[index] != null) {
					...
					@SuppressWarnings("unchecked") MutableObjectIterator<X> iter = (MutableObjectIterator<X>) this.localStrategies[index]
						.getIterator();
					in = iter;
				} else {
	...
	public static String constructLogString(String message, String taskName, AbstractInvokable parent) {
		return message + ":  " + taskName + " (" + (parent.getEnvironment().getTaskInfo().getIndexOfThisSubtask() + 1) + '/' + parent.getEnvironment()
	...
	public static <T> Collector<T> getOutputCollector(AbstractInvokable task, TaskConfig config, ClassLoader cl,
		...
		for(int i = 0; i < numOutputs; i++) {
			...
			final RecordWriter<SerializationDelegate<T>> recordWriter = new RecordWriterBuilder().setChannelSelector(oe)
				.setTaskName(task.getEnvironment().getTaskInfo().getTaskName()).build(task.getEnvironment().getWriter(outputOffset + i));
	...
	public static void openUserCode(Function stub, Configuration parameters) throws Exception {
		try {
			...
			throw new Exception(
				"The user defined 'open(Configuration)' method in " + stub.getClass().toString() + " caused an exception: " + t.getMessage(), t);
		}
	}
	...
	public static <T> T instantiateUserCode(TaskConfig config, ClassLoader cl, Class<? super T> superClass) {
		try {
			...
			if(superClass != null && !superClass.isAssignableFrom(stub.getClass())) {
				throw new RuntimeException(
					"The class '" + stub.getClass().getName() + "' is not a subclass of '" + superClass.getName() + "' as is required.");
			}
			return stub;
		} catch(ClassCastException ccex) {
