...
class NettyClient {
	...
	ChannelFuture connect(final InetSocketAddress serverSocketAddress) {
		...
		bootstrap.handler(new ChannelInitializer<SocketChannel>() {
			...
			public void initChannel(SocketChannel channel) throws Exception {
				...
				if(clientSSLFactory != null) {
					SslHandler sslHandler = clientSSLFactory
						.createNettySSLHandler(channel.alloc(), serverSocketAddress.getAddress().getCanonicalHostName(),
		...
		try {
			...
			if((e.getCause() instanceof java.net.SocketException && e.getCause().getMessage().equals("Too many open files")) || (e
				.getCause() instanceof ChannelException && e.getCause().getCause() instanceof java.net.SocketException && e.getCause().getCause()
				.getMessage().equals("Too many open files"))) {
