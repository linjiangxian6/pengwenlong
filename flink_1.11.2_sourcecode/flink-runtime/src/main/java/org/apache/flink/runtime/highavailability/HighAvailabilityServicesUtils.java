...
public class HighAvailabilityServicesUtils {
	...
	public static HighAvailabilityServices createHighAvailabilityServices(Configuration configuration, Executor executor,
		...
		// TODO 注释：获取 HA 模式, 一般，我们都是设置 zookeeper 模式
		// TODO 注释： 在 fink-conf.yaml 配置文件中，我们会去配置：high-availability = zookeeper
		...
		switch(highAvailabilityMode) {
			...
			case NONE:
				...
				// TODO 注释： 如果不是高可用：StandaloneHaServices
			...
			case ZOOKEEPER:
				...
				// TODO 注释： 获取 BlobStoreService
				BlobStoreService blobStoreService = BlobUtils.createBlobStoreFromConfig(configuration);

				// TODO 注释： 创建 ZooKeeperHaServices
				// TODO 注释： 对象的内部，包装了一个 ZooKeeper 的实例对象
				// TODO 注释： 内部的 zk 代码实现： 用的 curator
	...
	public static Tuple2<String, Integer> getJobManagerAddress(Configuration configuration) throws ConfigurationException {
		...
		if(hostname == null) {
			throw new ConfigurationException(
				"Config parameter '" + JobManagerOptions.ADDRESS + "' is missing (hostname/address of JobManager to connect to).");
		}

		if(port <= 0 || port >= 65536) {
			throw new ConfigurationException(
				"Invalid value for '" + JobManagerOptions.PORT + "' (port of the JobManager actor system) : " + port + ".  it must be greater than 0 and less than 65536.");
		}
	...
	public static Path getClusterHighAvailableStoragePath(Configuration configuration) {
		...
		// TODO 注释： 获取 HA 模式中，存储在 HDFS 上的数据的路径
		// TODO 注释： high-availability.storageDir
		...
		// TODO 注释： path 封装 storagePath
		final Path path;
		try {
			path = new Path(storagePath);
		} catch(Exception e) {
			throw new IllegalConfigurationException(
				"Invalid path for highly available storage (" + HighAvailabilityOptions.HA_STORAGE_PATH.key() + ')', e);
		}

		final String clusterId = configuration.getValue(HighAvailabilityOptions.HA_CLUSTER_ID);

		// TODO 注释： clusterStoragePath 封装 path
		...
		try {
			...
			throw new IllegalConfigurationException(String.format(
				...
				storagePath, clusterId, HighAvailabilityOptions.HA_CLUSTER_ID.key()), e);
		}
	...
	private static HighAvailabilityServices createCustomHAServices(Configuration config, Executor executor) throws FlinkException {
		...
		try {
			...
			throw new FlinkException(String.format("Could not create the ha services from the instantiated HighAvailabilityServicesFactory %s.",
				highAvailabilityServicesFactory.getClass().getName()), e);
		}
	}
	...
	private static HighAvailabilityServicesFactory loadCustomHighAvailabilityServicesFactory(
		...
		return InstantiationUtil.instantiate(highAvailabilityServicesFactoryClassName, HighAvailabilityServicesFactory.class, classLoader);
	}
	...
	private static ClientHighAvailabilityServices createCustomClientHAServices(Configuration config) throws FlinkException {
		...
		try {
			...
			throw new FlinkException(String
				.format("Could not create the client ha services from the instantiated HighAvailabilityServicesFactory %s.",
					highAvailabilityServicesFactory.getClass().getName()), e);
		}
	}
	...
	public enum AddressResolution {
		TRY_ADDRESS_RESOLUTION, NO_ADDRESS_RESOLUTION
	}
}
