...
public abstract class RestServerEndpoint implements AutoCloseableAsync {
	 ...
	 */
	protected abstract List<Tuple2<RestHandlerSpecification, ChannelInboundHandler>> initializeHandlers(
	...
	public final void start() throws Exception {
		synchronized(lock) {
			...
			// TODO 注释： 针对所有的 Handlers 进行排序，排序规则：RestHandlerUrlComparator
	...
	public CompletableFuture<Void> closeAsync() {
		synchronized(lock) {
			...
			if(state == State.RUNNING) {
				final CompletableFuture<Void> shutDownFuture = FutureUtils.composeAfterwards(closeHandlersAsync(), this::shutDownInternal);

				shutDownFuture.whenComplete((Void ignored, Throwable throwable) -> {
	...
	private FutureUtils.ConjunctFuture<Void> closeHandlersAsync() {
		return FutureUtils.waitForAll(handlers.stream().map(tuple -> tuple.f1).filter(handler -> handler instanceof AutoCloseableAsync)
			.map(handler -> ((AutoCloseableAsync) handler).closeAsync()).collect(Collectors.toList()));
	}
			...
			for(RestAPIVersion supportedAPIVersion : headers.getSupportedAPIVersions()) {
				...
				if(!isNewEndpoint) {
					throw new FlinkRuntimeException(String
						.format("REST handler registration overlaps with another registration for: version=%s, method=%s, url=%s.",
	...
	public static final class RestHandlerUrlComparator implements Comparator<Tuple2<RestHandlerSpecification, ChannelInboundHandler>>, Serializable {
		...
		public int compare(Tuple2<RestHandlerSpecification, ChannelInboundHandler> o1, Tuple2<RestHandlerSpecification, ChannelInboundHandler> o2) {
			...
			if(urlComparisonResult != 0) {
				...
				return API_VERSION_ORDER.compare(Collections.min(o1.f0.getSupportedAPIVersions()), Collections.min(o2.f0.getSupportedAPIVersions()));
			}
		}
