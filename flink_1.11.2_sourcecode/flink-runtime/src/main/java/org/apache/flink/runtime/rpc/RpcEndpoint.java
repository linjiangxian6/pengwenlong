...
/**
 * // TODO 注释： RpcEndpoint 是 业务逻辑的 载体， 可以认为是 ActorSystem 中的 Actor
 * // TODO 注释： RpcEndpoint 的位置透明的引用 RpcGateway
 * // TODO 注释： RpcEndpoint 你就把他看做值 Akka 中的 Actor
...
public abstract class RpcEndpoint implements RpcGateway, AutoCloseableAsync {
	 ...
	 *                   then the rpc endpoint will automatically terminate.
	 *
	 *  // TODO 注释： 只要当前的 RpcEndpoint 被实例化成功了之后，就会调用 onStart() 方法
	 */
	 ...
	 * occurs this future is completed exceptionally
	 *
	 * // TODO 注释： 当当前这个 RpcEndpoint 需要被销毁的时候：在销毁之前，调用这个方法执行一次
	 */
	...
	public <C extends RpcGateway> C getSelfGateway(Class<C> selfGatewayType) {
		if(selfGatewayType.isInstance(rpcServer)) {
			@SuppressWarnings("unchecked") C selfGateway = ((C) rpcServer);
