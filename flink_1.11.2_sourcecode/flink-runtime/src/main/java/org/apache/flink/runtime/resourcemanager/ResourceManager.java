...
/**
 * // TODO 注释： ResourceManager的实现。资源经理负责资源的取消/分配和簿记。
 ...
 * <ul>
 *     // TODO 注释： registerJobManager(JobMasterId, ResourceID, String, JobID, Time) 方法负责在 ResourceManager 注册 JobMaster
 ...
 *     // TODO 注释： requestSlot(JobMasterId, SlotRequest, Time) 负责从 ResourceManager 申请
...
public abstract class ResourceManager<WorkerType extends ResourceIDRetrievable> extends FencedRpcEndpoint<ResourceManagerId> implements ResourceManagerGateway, LeaderContender {
	...
	/**
	 * // TODO 注释： 所有已经注册过的 JobMaster, 也可以理解成 JobManager, 通过 JobID 映射管理
	...
	/**
	 * // TODO 注释： 所有已经注册过的 JobMaster, 也可以理解成 JobManager, 通过 ResourceID 映射管理
	...
	private void startResourceManagerServices() throws Exception {
		try {
			...
			// TODO 注释： 其实在 Standalone 模式下，什么也没做
			initialize();

			// TODO 注释： 注意这个 this 对象
			// TODO 注释： 执行选举，成功之后，调用 leaderElectionService.isLeader()
			// TODO 注释： this = ResourceManager
	...
	public CompletableFuture<Acknowledge> requestSlot(JobMasterId jobMasterId, SlotRequest slotRequest, final Time timeout) {
		...
		if(null != jobManagerRegistration) {
			...
			// TODO 注释： 判断申请 slot 的 JobMaster 和 注册的 Job 的 Master 地址是否一样
			// TODO 注释： 如果不一样，则放弃。防止因为 JobMaster 迁移导致申请了双倍的slot导致资源浪费
	...
	private RegistrationResponse registerTaskExecutorInternal(TaskExecutorGateway taskExecutorGateway,
		...
		// TODO 注释： 获取 TaskExecutor 的 ResourceID
		...
		if(oldRegistration != null) {
			...
			// TODO 注释： 从 SlotManager 中移除旧的 TaskManager
		...
		if(newWorker == null) {
			 ...
			 *  5、如果是HBase， 除了 regionserve 上线之后，向 HMaster 注册以外， 还会把自己的信息，写到 ZK 里面
			 */
	...
	protected void closeTaskManagerConnection(final ResourceID resourceID, final Exception cause) {
		...
		// TODO 注释： 获取该 TaskManager 的注册信息对象： WorkerRegistration， 同时执行移除
	...
	private CompletableFuture<Boolean> tryAcceptLeadership(final UUID newLeaderSessionID) {
		...
		if(leaderElectionService.hasLeadership(newLeaderSessionID)) {
			...
			// TODO 注释： 生成一个 ResourceManagerID
