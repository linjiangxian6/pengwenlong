...
public class DispatcherRestEndpoint extends WebMonitorEndpoint<DispatcherGateway> {
	...
	protected List<Tuple2<RestHandlerSpecification, ChannelInboundHandler>> initializeHandlers(final CompletableFuture<String> localAddressFuture) {
		 ...
		 */
		//TODO ***** initializeHandlers 实现类WebMonitorEndpoint
	...
	protected Collection<Tuple2<RestHandlerSpecification, ChannelInboundHandler>> initializeWebSubmissionHandlers(
		...
		if(restConfiguration.isWebSubmitEnabled()) {
			try {
				...
				// TODO 注释： 加载得到： WebSubmissionExtension
				webSubmissionExtension = WebMonitorUtils
					.loadWebSubmissionExtension(leaderRetriever, timeout, responseHeaders, localAddressFuture, uploadDir, executor,
	...
	protected CompletableFuture<Void> shutDownInternal() {
		...
		shutdownFuture.whenComplete((Void ignored, Throwable throwable) -> {
			webSubmissionExtension.closeAsync().whenComplete((Void innerIgnored, Throwable innerThrowable) -> {
