...
public class BlobUtils {
	...
	public static BlobStoreService createBlobStoreFromConfig(Configuration config) throws IOException {
		...
		// TODO 注释： 如果开启了高可用（需要开启备份服务来支持高可用）
	 ...
	 * @throws IOException thrown if an I/O error occurs while writing to the output
	 *                     stream
	 */
	 ...
	 * @throws IOException Thrown if I/O error occurs while reading from the stream or the stream cannot offer enough data.
	 */
	 ...
	 */
	static void moveTempFileToStore(File incomingFile, @Nullable JobID jobId, BlobKey blobKey, File storageFile, Logger log,
		@Nullable BlobStore blobStore) throws IOException {
		...
		try {
			...
			if(!storageFile.exists()) {
				try {
					...
					log.warn(
						"Detected concurrent file modifications. This should only happen if multiple" + "BlobServer use the same storage directory.");
