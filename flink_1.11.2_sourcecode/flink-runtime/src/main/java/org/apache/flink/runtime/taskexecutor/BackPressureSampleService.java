...
/**
 * // TODO 注释： 采样任务是否多次背压。样本总数除以背压样本数即可得出背压比。
...
public class BackPressureSampleService {
	...
	public CompletableFuture<Double> sampleTaskBackPressure(BackPressureSampleableTask task) {
		...
		return sampleTaskBackPressure(checkNotNull(task), numSamples, new ArrayList<>(numSamples), new CompletableFuture<>());
	}

	private CompletableFuture<Double> sampleTaskBackPressure(BackPressureSampleableTask task, int remainingNumSamples, List<Boolean> taskBackPressureSamples,
		...
		if(task.isRunning() && remainingNumSamples > 1) {
			scheduledExecutor
				.schedule(() -> sampleTaskBackPressure(task, remainingNumSamples - 1, taskBackPressureSamples, resultFuture), delayBetweenSamples.getSize(),
