...
public class OperatorTranslation {
	...
	private <T> Operator<T> translate(DataSet<T> dataSet) {
		...
		if(dataSet instanceof DataSource) {
			...
			throw new InvalidProgramException(
				"A data set that is part of a delta iteration was used as a sink or action." + " Did you forget to close the iteration?");
		} else {
	...
	private <T> BulkIterationBase<T> translateBulkIteration(BulkIterationResultSet<?> untypedIterationEnd) {
		...
		BulkIterationBase<T> iterationOperator = new BulkIterationBase<>(
			new UnaryOperatorInformation<>(iterationEnd.getType(), iterationEnd.getType()), "Bulk Iteration");
	...
	private <D, W> DeltaIterationBase<D, W> translateDeltaIteration(DeltaIterationResultSet<?, ?> untypedIterationEnd) {
		...
		DeltaIterationBase<D, W> iterationOperator = new DeltaIterationBase<>(
			new BinaryOperatorInformation<>(iterationEnd.getType(), iterationEnd.getWorksetType(), iterationEnd.getType()),
