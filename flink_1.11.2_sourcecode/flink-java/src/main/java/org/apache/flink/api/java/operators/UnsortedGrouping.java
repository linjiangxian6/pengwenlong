...
public class UnsortedGrouping<T> extends Grouping<T> {
	...
	public <R> GroupReduceOperator<T, R> reduceGroup(GroupReduceFunction<T, R> reducer) {
		...
		TypeInformation<R> resultType = TypeExtractor
			.getGroupReduceReturnTypes(reducer, this.getInputDataSet().getType(), Utils.getCallLocationName(), true);
	...
	public <R> GroupCombineOperator<T, R> combineGroup(GroupCombineFunction<T, R> combiner) {
		...
		TypeInformation<R> resultType = TypeExtractor
			.getGroupCombineReturnTypes(combiner, this.getInputDataSet().getType(), Utils.getCallLocationName(), true);
	...
	public <K> SortedGrouping<T> sortGroup(KeySelector<T, K> keySelector, Order order) {
		...
		SortedGrouping<T> sg = new SortedGrouping<T>(this.inputDataSet, this.keys,
			new Keys.SelectorFunctionKeys<T, K>(keySelector, this.inputDataSet.getType(), keyType), order);
		sg.customPartitioner = getCustomPartitioner();
		return sg;
	}
