...
public abstract class DataSet<T> {
	...
	public <K extends Comparable<K>> PartitionOperator<T> partitionByHash(KeySelector<T, K> keyExtractor) {
		...
		return new PartitionOperator<>(this, PartitionMethod.HASH, new Keys.SelectorFunctionKeys<>(clean(keyExtractor), this.getType(), keyType),
	...
	public <K extends Comparable<K>> PartitionOperator<T> partitionByRange(KeySelector<T, K> keyExtractor) {
		final TypeInformation<K> keyType = TypeExtractor.getKeySelectorTypes(keyExtractor, getType());
		return new PartitionOperator<>(this, PartitionMethod.RANGE, new Keys.SelectorFunctionKeys<>(clean(keyExtractor), this.getType(), keyType),
			Utils.getCallLocationName());
	}
	...
	public <K> PartitionOperator<T> partitionCustom(Partitioner<K> partitioner, String field) {
		return new PartitionOperator<>(this, new Keys.ExpressionKeys<>(new String[]{field}, getType()), clean(partitioner),
			Utils.getCallLocationName());
	}
	...
	public <K extends Comparable<K>> PartitionOperator<T> partitionCustom(Partitioner<K> partitioner, KeySelector<T, K> keyExtractor) {
		final TypeInformation<K> keyType = TypeExtractor.getKeySelectorTypes(keyExtractor, getType());
		return new PartitionOperator<>(this, new Keys.SelectorFunctionKeys<>(keyExtractor, getType(), keyType), clean(partitioner),
			Utils.getCallLocationName());
	}
	...
	public <K> SortPartitionOperator<T> sortPartition(KeySelector<T, K> keyExtractor, Order order) {
		final TypeInformation<K> keyType = TypeExtractor.getKeySelectorTypes(keyExtractor, getType());
		return new SortPartitionOperator<>(this, new Keys.SelectorFunctionKeys<>(clean(keyExtractor), getType(), keyType), order,
			Utils.getCallLocationName());
	}
	...
	private <X extends Tuple> DataSink<T> internalWriteAsCsv(Path filePath, String rowDelimiter, String fieldDelimiter, WriteMode wm) {
		...
		// TODO 注释： 指定输出 OutputFormat
		CsvOutputFormat<X> of = new CsvOutputFormat<>(filePath, rowDelimiter, fieldDelimiter);
		if(wm != null) {
			of.setWriteMode(wm);
		}

		// TODO 注释： 通过 output 方法往外输出： 具体的输出由： CsvOutputFormat 负责
		// TODO 注释： output 方法的结果，只是拿到一个 DataSink 的实例
	...
	public DataSink<T> output(OutputFormat<T> outputFormat) {
		...
		// TODO 注释： 构造一个如何做输出的 DataSink 实例，并没有执行真正的输出，因为压根没执行相关的数据读取和计算
