...
public class ExecutionEnvironment {
	 ...
	 */
	@PublicEvolving
	public ExecutionEnvironment(final PipelineExecutorServiceLoader executorServiceLoader, final Configuration configuration,
	...
	public <X> DataSource<X> readFileOfPrimitives(String filePath, Class<X> typeClass) {
		...
		return new DataSource<>(this, new PrimitiveInputFormat<>(new Path(filePath), typeClass), TypeExtractor.getForClass(typeClass),
			Utils.getCallLocationName());
	}
	...
	public <X> DataSource<X> readFileOfPrimitives(String filePath, String delimiter, Class<X> typeClass) {
		...
		return new DataSource<>(this, new PrimitiveInputFormat<>(new Path(filePath), delimiter, typeClass), TypeExtractor.getForClass(typeClass),
			Utils.getCallLocationName());
	}
	...
	public final <X> DataSource<X> fromElements(X... data) {
		...
		try {
			...
			throw new RuntimeException("Could not create TypeInformation for type " + data[0].getClass()
				.getName() + "; please specify the TypeInformation manually via " + "ExecutionEnvironment#fromElements(Collection, TypeInformation)",
				e);
		}
	...
	public static ExecutionEnvironment getExecutionEnvironment() {
		...
		return Utils.resolveFactory(threadLocalContextEnvironmentFactory, contextEnvironmentFactory)
			.map(ExecutionEnvironmentFactory::createExecutionEnvironment).orElseGet(ExecutionEnvironment::createLocalEnvironment);
	}
