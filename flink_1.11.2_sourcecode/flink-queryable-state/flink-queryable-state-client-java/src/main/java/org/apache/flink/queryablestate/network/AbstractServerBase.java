...
public abstract class AbstractServerBase<REQ extends MessageBody, RESP extends MessageBody> {
	 ...
	 */
	protected AbstractServerBase(final String serverName, final String bindAddress, final Iterator<Integer> bindPortIterator, final Integer numEventLoopThreads,
	...
	private ExecutorService createQueryExecutor() {
		ThreadFactory threadFactory = new ThreadFactoryBuilder().setDaemon(true).setNameFormat("Flink " + getServerName() + " Thread %d").build();
	...
	public void start() throws Throwable {
		Preconditions.checkState(serverAddress == null && serverShutdownFuture.get() == null, serverName + " is already running @ " + serverAddress + ". ");
	...
	private boolean attemptToBind(final int port) throws Throwable {
		...
		this.bootstrap = new ServerBootstrap().localAddress(bindAddress, port).group(nioGroup).channel(NioServerSocketChannel.class)
			.option(ChannelOption.ALLOCATOR, bufferPool).childOption(ChannelOption.ALLOCATOR, bufferPool).childHandler(new ServerChannelInitializer<>(handler));
	...
	private static final class ServerChannelInitializer<REQ extends MessageBody, RESP extends MessageBody> extends ChannelInitializer<SocketChannel> {
		...
		protected void initChannel(SocketChannel channel) throws Exception {
			channel.pipeline().addLast(new ChunkedWriteHandler()).addLast(new LengthFieldBasedFrameDecoder(Integer.MAX_VALUE, 0, 4, 0, 4))
