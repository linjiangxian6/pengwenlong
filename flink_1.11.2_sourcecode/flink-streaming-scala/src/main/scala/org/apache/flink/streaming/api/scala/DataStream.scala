...
@Public class DataStream[T](stream: JavaStream[T]) {
   ...
   */
  @deprecated
  @PublicEvolving def getExecutionEnvironment: StreamExecutionEnvironment = new StreamExecutionEnvironment(stream.getExecutionEnvironment)
   ...
   */
  @deprecated
  @PublicEvolving def getType(): TypeInformation[T] = stream.getType()
   ...
   */
  @deprecated
  @PublicEvolving def getParallelism = stream.getParallelism
   ...
   */
  @deprecated
  @PublicEvolving def getExecutionConfig = stream.getExecutionConfig
  
  /**
   * Returns the ID of the DataStream.
   */
  @Internal private[flink] def getId = stream.getId()
   ...
   */
  def executionEnvironment: StreamExecutionEnvironment = new StreamExecutionEnvironment(stream.getExecutionEnvironment())
  ...
  def setParallelism(parallelism: Int): DataStream[T] = {
    stream match {
      ...
      case _ => throw new UnsupportedOperationException("Operator " + stream + " cannot set the parallelism.")
    }
    this
  }
  ...
  def setMaxParallelism(maxParallelism: Int): DataStream[T] = {
    stream match {
      ...
      case _ => throw new UnsupportedOperationException("Operator " + stream + " cannot set the maximum" + "paralllelism")
    }
   ...
   */
  @PublicEvolving def minResources: ResourceSpec = stream.getMinResources()
  
  /**
   * Returns the preferred resources of this operation.
   */
  @PublicEvolving def preferredResources: ResourceSpec = stream.getPreferredResources()
   ...
   */
  @deprecated
  @PublicEvolving def getName: String = name
   ...
   */
  @PublicEvolving def uid(uid: String): DataStream[T] = javaStream match {
    case stream: SingleOutputStreamOperator[T] => asScalaStream(stream.uid(uid))
    case _ => throw new UnsupportedOperationException("Only supported for operators.")
      this
  }
  
  @PublicEvolving def getSideOutput[X: TypeInformation](tag: OutputTag[X]): DataStream[X] = javaStream match {
    case stream: SingleOutputStreamOperator[X] => asScalaStream(stream.getSideOutput(tag: OutputTag[X]))
  }
   ...
   */
  @PublicEvolving def setUidHash(hash: String): DataStream[T] = javaStream match {
    case stream: SingleOutputStreamOperator[T] => asScalaStream(stream.setUidHash(hash))
    case _ => throw new UnsupportedOperationException("Only supported for operators.")
      this
  }
   ...
   *
   */
  @PublicEvolving def disableChaining(): DataStream[T] = {
    stream match {
   ...
   *
   */
  @PublicEvolving def startNewChain(): DataStream[T] = {
    stream match {
   ...
   */
  @PublicEvolving def slotSharingGroup(slotSharingGroup: String): DataStream[T] = {
    stream match {
   ...
   *
   */
  def union(dataStreams: DataStream[T]*): DataStream[T] = asScalaStream(stream.union(dataStreams.map(_.javaStream): _*))
   ...
   */
  @PublicEvolving def connect[R](broadcastStream: BroadcastStream[R]): BroadcastConnectedStream[T, R] = asScalaStream(stream.connect(broadcastStream))
   ...
   *
   */
  @deprecated("use [[DataStream.keyBy(KeySelector)]] instead") def keyBy(fields: Int*): KeyedStream[T, JavaTuple] = asScalaStream(stream.keyBy(fields: _*))
  
  /**
   * Groups the elements of a DataStream by the given field expressions to
   * be used with grouped operators like grouped reduce or grouped aggregations.
   */
  @deprecated("use [[DataStream.keyBy(KeySelector)]] instead") def keyBy(firstField: String, otherFields: String*): KeyedStream[T, JavaTuple] = asScalaStream(
   ...
   */
  @deprecated("Use [[DataStream.partitionCustom(Partitioner, Function1)]] instead") def partitionCustom[K: TypeInformation](partitioner: Partitioner[K],
    field: Int): DataStream[T] = asScalaStream(stream.partitionCustom(partitioner, field))
   ...
   */
  @deprecated("Use [[DataStream.partitionCustom(Partitioner, Function1)]] instead") def partitionCustom[K: TypeInformation](partitioner: Partitioner[K],
    field: String): DataStream[T] = asScalaStream(stream.partitionCustom(partitioner, field))
   ...
   */
  @PublicEvolving def broadcast(broadcastStateDescriptors: MapStateDescriptor[_, _]*): BroadcastStream[T] = {
   ...
   */
  @PublicEvolving def global: DataStream[T] = asScalaStream(stream.global())
  
  /**
   * Sets the partitioning of the DataStream so that the output tuples
   * are shuffled to the next component.
   */
  @PublicEvolving def shuffle: DataStream[T] = asScalaStream(stream.shuffle())
   ...
   */
  @PublicEvolving def rescale: DataStream[T] = asScalaStream(stream.rescale())
   ...
   */
  @PublicEvolving def iterate[R](stepFunction: DataStream[T] => (DataStream[T], DataStream[R]), maxWaitTimeMillis: Long = 0): DataStream[R] = {
   ...
   *
   */
  @PublicEvolving def iterate[R, F: TypeInformation](stepFunction: ConnectedStreams[T, F] => (DataStream[F], DataStream[R]),
   ...
   *                   in the stream.
   */
  @PublicEvolving def process[R: TypeInformation](processFunction: ProcessFunction[T, R]): DataStream[R] = {
   ...
   */
  @PublicEvolving def windowAll[W <: Window](assigner: WindowAssigner[_ >: T, W]): AllWindowedStream[T, W] = {
   ...
   */
  @deprecated
  @PublicEvolving def assignTimestampsAndWatermarks(assigner: AssignerWithPeriodicWatermarks[T]): DataStream[T] = {
   ...
   */
  @deprecated
  @PublicEvolving def assignTimestampsAndWatermarks(assigner: AssignerWithPunctuatedWatermarks[T]): DataStream[T] = {
   ...
   */
  @PublicEvolving def assignAscendingTimestamps(extractor: T => Long): DataStream[T] = {
   ...
   * @deprecated Please use side output instead.
   */
  @deprecated def split(fun: T => TraversableOnce[String]): SplitStream[T] = {
    if (fun == null) {
   ...
   *
   */
  @PublicEvolving def print(): DataStreamSink[T] = stream.print()
   ...
   */
  @PublicEvolving def printToErr() = stream.printToErr()
   ...
   * @return The closed DataStream.
   */
  @PublicEvolving def print(sinkIdentifier: String): DataStreamSink[T] = stream.print(sinkIdentifier)
   ...
   * @return The closed DataStream.
   */
  @PublicEvolving def printToErr(sinkIdentifier: String) = stream.printToErr(sinkIdentifier)
   ...
   */
  @Deprecated
  @PublicEvolving def writeAsText(path: String): DataStreamSink[T] = stream.writeAsText(path)
   ...
   */
  @Deprecated
  @PublicEvolving def writeAsText(path: String, writeMode: FileSystem.WriteMode): DataStreamSink[T] = {
    if (writeMode != null) {
   ...
   */
  @Deprecated
  @PublicEvolving def writeAsCsv(path: String): DataStreamSink[T] = {
    writeAsCsv(path, null, ScalaCsvOutputFormat.DEFAULT_LINE_DELIMITER, ScalaCsvOutputFormat.DEFAULT_FIELD_DELIMITER)
  }
   ...
   */
  @Deprecated
  @PublicEvolving def writeAsCsv(path: String, writeMode: FileSystem.WriteMode): DataStreamSink[T] = {
    writeAsCsv(path, writeMode, ScalaCsvOutputFormat.DEFAULT_LINE_DELIMITER, ScalaCsvOutputFormat.DEFAULT_FIELD_DELIMITER)
  }
   ...
   */
  @Deprecated
  @PublicEvolving def writeAsCsv(path: String, writeMode: FileSystem.WriteMode, rowDelimiter: String, fieldDelimiter: String): DataStreamSink[T] = {
   ...
   */
  @PublicEvolving def writeUsingOutputFormat(format: OutputFormat[T]): DataStreamSink[T] = {
   ...
   */
  @PublicEvolving def writeToSocket(hostname: String, port: Integer, schema: SerializationSchema[T]): DataStreamSink[T] = {
   ...
   */
  @PublicEvolving def transform[R: TypeInformation](operatorName: String, operator: OneInputStreamOperator[T, R]): DataStream[R] = {
