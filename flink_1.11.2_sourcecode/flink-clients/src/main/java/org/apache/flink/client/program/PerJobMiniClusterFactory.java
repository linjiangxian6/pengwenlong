...
public final class PerJobMiniClusterFactory {
	...
	public static PerJobMiniClusterFactory createWithFactory(Configuration configuration,
	...
	private PerJobMiniClusterFactory(Configuration configuration,
		Function<? super MiniClusterConfiguration, ? extends MiniCluster> miniClusterFactory) {
	...
	private MiniClusterConfiguration getMiniClusterConfig(int maximumParallelism) {
		...
		int numSlotsPerTaskManager = configuration.getOptional(TaskManagerOptions.NUM_TASK_SLOTS).orElseGet(
			() -> maximumParallelism > 0 ? MathUtils.divideRoundUp(maximumParallelism, numTaskManagers) : TaskManagerOptions.NUM_TASK_SLOTS
				.defaultValue());

		return new MiniClusterConfiguration.Builder().setConfiguration(configuration).setNumTaskManagers(numTaskManagers)
			.setRpcServiceSharing(RpcServiceSharing.SHARED).setNumSlotsPerTaskManager(numSlotsPerTaskManager).build();
	}
