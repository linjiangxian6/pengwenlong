...
public class StreamContextEnvironment extends StreamExecutionEnvironment {
	...
	private JobExecutionResult getJobExecutionResult(final JobClient jobClient) throws Exception {
		...
		if(getConfiguration().getBoolean(DeploymentOptions.ATTACHED)) {
			CompletableFuture<JobExecutionResult> jobExecutionResultFuture = jobClient.getJobExecutionResult(getUserClassloader());
			...
			if(getConfiguration().getBoolean(DeploymentOptions.SHUTDOWN_IF_ATTACHED)) {
				...
				jobExecutionResultFuture.whenComplete(
					(ignored, throwable) -> ShutdownHookUtil.removeShutdownHook(shutdownHook, StreamContextEnvironment.class.getSimpleName(), LOG));
			}
	...
	public static void setAsContext(final PipelineExecutorServiceLoader executorServiceLoader, final Configuration configuration,
		final ClassLoader userCodeClassLoader, final boolean enforceSingleJobExecution, final boolean suppressSysout) {
