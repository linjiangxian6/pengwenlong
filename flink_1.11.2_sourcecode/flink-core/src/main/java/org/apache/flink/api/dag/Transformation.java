...
/**
 * // TODO 注释： {@code Transformation}表示创建DataStream的操作。
 * // TODO 注释： 每个DataStream都有一个基础的{@code Transformation}，它是该DataStream的起源。
 ...
 *
 * // TODO 注释： API操作（例如DataStream＃map）在下面创建了{@code Transformation} s的树。
 * // TODO 注释： 当要执行流程序时，使用StreamGraphGenerator将该图转换为StreamGraph。
 * <p>API operations such as DataStream#map create  a tree of {@code Transformation}s underneath.
 * When the stream program is to be executed this graph is translated to a StreamGraph using StreamGraphGenerator.
 *
 * // TODO 注释： {@ code Transformation}不一定与运行时的物理操作相对应。 某些操作仅是逻辑概念。
 * // TODO 注释： 例如联合，拆分/选择数据流，分区。
 * <p>A {@code Transformation} does not necessarily correspond to a physical operation at runtime.
 * Some operations are only logical concepts. Examples of this are union, split/select data stream, partitioning.
 *
...
public abstract class Transformation<T> {
	 ...
	 *  -
	 *  从 source 到各种 map, union 再到 sink 操作全部被映射成了 Transformation。通过看它的子类可以看到；
	 *  -
	 *  其实可以理解为就是所有算子的公共父类；
	 *  -
	 *  所有的算子都是 DataStream 的一个方法，每次调用一个方法(也就是调用一个算子)，
	 *  DataStream 这个类就通过 transform 这个方法，把这个算子转换成 Transformation，
	 *  然后放到 StreamExecutionEnvironment 执行环境中的 transformations list 容器中，
	 *  最终生成 StreamGraph 的时候， 会根据这个容器中的操作生成；
	 */
	...
	public TypeInformation<T> getOutputType() {
		if(outputType instanceof MissingTypeInfo) {
			...
			throw new InvalidTypesException("The return type of function '" + typeInfo
				.getFunctionName() + "' could not be determined automatically, due to type erasure. " + "You can give type information hints by using the returns(...) " + "method on the result of the transformation call, or by letting " + "your function implement the 'ResultTypeQueryable' " + "interface.",
	...
	public String toString() {
		return getClass()
			.getSimpleName() + "{" + "id=" + id + ", name='" + name + '\'' + ", outputType=" + outputType + ", parallelism=" + parallelism + '}';
	}
