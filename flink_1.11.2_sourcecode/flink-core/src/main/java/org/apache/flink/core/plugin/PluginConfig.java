...
public class PluginConfig {
	...
	public static PluginConfig fromConfiguration(Configuration configuration) {
		...
		// TODO 注释：
		return new PluginConfig(getPluginsDir().map(File::toPath), CoreOptions.getPluginParentFirstLoaderPatterns(configuration));
	}

	// TODO 注释： 确保 plugins 文件夹存在
	public static Optional<File> getPluginsDir() {

		// TODO 注释： FLINK_PLUGINS_DIR = plugins
		String pluginsDir = System.getenv().getOrDefault(ConfigConstants.ENV_FLINK_PLUGINS_DIR, ConfigConstants.DEFAULT_FLINK_PLUGINS_DIRS);
